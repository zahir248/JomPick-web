...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($username) || empty($phoneNumber) || empty($icNumber) || empty($emailAddress) || empty($fullName)) {
        header("Location: ../manager-update.php?jpid=$jpid&error=Input Can't Be Empty");
        exit();
    }

    // Validate phone number
    if (!preg_match("/^[0-9]{10,11}$/", $phoneNumber)) {
        header("Location: ../manager-update.php?jpid=$jpid&error=Invalid Phone Number");
        exit();
    }

    // Validate email address
    if (!filter_var($emailAddress, FILTER_VALIDATE_EMAIL)) {
        header("Location: ../manager-update.php?jpid=$jpid&error=Invalid Email Address");
        exit();
    }

    // Validate IC number (Assuming a simple format)
    if (!preg_match("/^[0-9]{12}$/", $icNumber)) {
        header("Location: ../manager-update.php?jpid=$jpid&error=Invalid IC Number");
        exit();
    }
    ...
    if ($result) {
        ...
        header("Location: ../manager-update.php?jpid=$jpid&success=Manager Details Updated Successful");
        exit;
    } else {
